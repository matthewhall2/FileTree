...
# extracted the files.
EXAMPLE_PATH = os.path.join('C:\\Users\\hallm\\Desktop\\h\\csc148\\assignments'
                            '\\a2\\example-directory', 'workshop')
...
def test_papers():
    ...
    print(paper_tree._subtrees)
    assert paper_tree._subtrees[0]._name == 'FLP'
    assert paper_tree._subtrees[0]._subtrees[0]._name == 'other'
    assert paper_tree._subtrees[0]._subtrees[0]._subtrees[0]._name \
           ==  'language agnostic approaches'
    assert paper_tree._subtrees[0]._subtrees[0]._subtrees[0]._subtrees[0]._name\
           == 'Separation of Introductory Programming and Language Instruction'
    assert paper_tree._subtrees[0]._subtrees[0]._subtrees[0]._subtrees[0]._subtrees == []
    assert len(paper_tree._subtrees[0]._subtrees) == 4
    assert paper_tree._count_leaves() == 483
    assert paper_tree._subtrees[2]._name == 'Students'
    l = len(paper_tree._subtrees[2]._subtrees)
    assert paper_tree._subtrees[2]._subtrees[l - 1]._name == 'prior knowledge'
    assert len(paper_tree._subtrees[2]._subtrees[l - 1]._subtrees) == 6
    for i in range(5):
        assert len(paper_tree._subtrees[2]._subtrees[l - 1]._subtrees[i]._subtrees) == 0
    assert paper_tree._subtrees[2]._subtrees[l - 1]._subtrees[4].data_size == 0
    assert len(paper_tree._subtrees[2]._subtrees[l - 1]._subtrees[5]._subtrees) != 0

def test_empty_papers_tree()-> None:
    tree = PaperTree(None, [], citations=0)
    assert tree._name is None
    assert tree._subtrees == []

def test_papers_tree_categories()-> None:
    tree = PaperTree('CS1', [], all_papers=True, by_year=False)
    assert len(tree._subtrees) == 9
