...
class PaperTree(TMTree):
    ...
    def __init__(self, name: str, subtrees: List[TMTree], authors: str = '',
        ...
        if all_papers:
            ...
            l = _build_tree_from_dict(d)
            TMTree.__init__(self, name, l)
        else:
...
def _load_papers_to_dict(by_year: bool = True) -> Dict:
    ...
    The last nested dictionary's value will be a tuple in the form
    (author, title, year, url, citations)
    """
    dict_ = {}
    ...
    j = 0
    for row in r:
        category_lst = row['Category'].split(':')
        author = row['Author']
        title = row['Title']
        year = row['Year']
        url = row['Url']
        ...
        for i in range(len(category_lst)):
            category_lst[i] = category_lst[i].strip()
        info = (title, author, url, int(citations), year)
        dict_ = _build_dic_from_list(category_lst, dict_, info, by_year, year)
    csvfile.close()
    print(dict_)
    return dict_


def _build_dic_from_list(lst: List, dict_: Dict, info: Tuple,
                         all_year: bool, year: str)-> Dict:
    """
    builds a nested dictionary with a lst of keys <lst> in <dict_>
    """
    if len(lst) == 1:
        if lst[0] not in dict_:
            dict_[lst[0]] = {}
        if () not in dict_[lst[0]]:
            dict_[lst[0]][()] = [info]
        else:
            dict_[lst[0]][()].append(info)
        return dict_
    else:
        if all_year:
            if year not in dict_:
                dict_[year] = ''
            if isinstance(dict_[year], Dict):
                dict_[year] = _build_dic_from_list(lst, dict_[year], info,
                                                   False, 0)
            else:
                dict_[year] = _build_dic_from_list(lst, {}, info, False, 0)
            return dict_
        else:
            if lst[0] not in dict_:
                dict_[lst[0]] = ''
            if isinstance(dict_[lst[0]], Dict):
                dict_[lst[0]] = _build_dic_from_list(lst[1:], dict_[lst[0]],
                                                     info, False, 0)
            else:
                dict_[lst[0]] = _build_dic_from_list(lst[1:], {}, info,
                                                     False, 0)
            return dict_
...
def _build_tree_from_dict(nested_dict: Dict) -> List[PaperTree]:
    ...
    if not isinstance(nested_dict, Dict):
        for items in nested_dict:
            tree = PaperTree(items[0], [], items[1], items[2], items[3], False)
            lst.append(tree)
        return lst
    else:
        ...
        #title = list(keys)[0]
        for items in keys:
            lst.extend(_build_tree_from_dict(nested_dict[items]))
            if items == ():
                title = nested_dict[items][0][0]
                items = title
                lst2.extend(lst)
            else:
                lst2.append(PaperTree(items, lst))
            lst = []
        return lst2
